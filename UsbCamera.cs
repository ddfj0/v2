...
//https://github.com/secile/UsbCamera/blob/master/UsbCamera.cs
...
namespace v2
{
    ...
    public class CUsbCamera
    {
        public String m_CameraName;

        private UsbCamera camera;
        private UsbCamera.VideoFormat[] formats;


        public String[] fnGetCamera()
        {
        ...
        public String[] fnGetForamt(Int32 index)
        {

            formats = UsbCamera.GetVideoFormat(index);

            String[] strFormat = new string[formats.Length];

            for (int i = 0; i < formats.Length; i++)
            {
                if (formats[i].Caps.Guid == DirectShow.DsGuid.FORMAT_VideoInfo)
                    strFormat[i] = formats[i].ToString();
                else
                    strFormat[i] = "unsurport - " + formats[i].ToString();
            }
        ...
        public bool fnStartCamera(Int32 cc, Int32 ff)
        {
            try
            {
                formats = UsbCamera.GetVideoFormat(cc);

                camera = new UsbCamera(cc, formats[ff]);
                camera.m_CameraName = m_CameraName;
                camera.Start();
                return true;
            }
            catch (Exception ee)
            {
                MessageBox.Show("打开摄像头出错。 " + ee.Message);
            }
            //fnSleep(3000);
            return false;
        }

        public Bitmap fnCapture()
        {
            Bitmap bmp = null;
            try
            {
        ...
        public void fnGetPicture(String fileName)
        {
            string[] devices = UsbCamera.FindDevices();
            ...
            // check format.
            UsbCamera.VideoFormat[] formats = UsbCamera.GetVideoFormat(index);
            ...
            bmp.Save(fileName, System.Drawing.Imaging.ImageFormat.Png);// image为要保存的图片

            camera.Stop();

        }
        public static void fnSleep(Int32 msec)
        {
            try
            {
                Thread.Sleep(msec);
